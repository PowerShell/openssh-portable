...
DWORD WINAPI
signal_test_thread_random_sleep(LPVOID lpParam)
{
	Sleep(200 + rand() % 300);
	return TRUE;
}
...
signal_test_wait_for_multiple_objects()
{
	...
	{
		TEST_START("Signal: APC wakeup with select object counts (WAIT_IO_COMPLETION_ENHANCED)");
		TEST_RESOURCES(TRUE);
		...
		for (int i = 0; i < objects_size; i++)
		{
			...
			if (select == 0 || select == 1 || select == MAXIMUM_WAIT_OBJECTS - 1 || select == MAXIMUM_WAIT_OBJECTS - 2)
			{
				...
				DWORD ret = wait_for_multiple_objects_enhanced(i + 1, hObjects, TRUE, 10000, TRUE);
	...
	{
		TEST_START("Signal: Wait-all on array on signaled objects (WAIT_OBJECT_0_ENHANCED)");
		TEST_RESOURCES(TRUE);

		for (int i = 0; i < objects_size; i++) SetEvent(hObjects[i]);
		DWORD ret = wait_for_multiple_objects_enhanced(objects_size, hObjects, TRUE, 10000, FALSE);
		ASSERT_INT_EQ(ret, WAIT_OBJECT_0_ENHANCED);
	...
	{
		TEST_START("Signal: Wait-all with one invalid object in positions 1-300 (WAIT_FAILED_ENHANCED)");
		TEST_RESOURCES(TRUE);
	...
	{
		TEST_START("Signal: Wait-all with object counts from 1-300 (WAIT_OBJECT_0_ENHANCED)");
		TEST_RESOURCES(TRUE);

		for (int i = 0; i < objects_size; i++) {
			for (int j = 0; j < objects_size; j++) SetEvent(hObjects[j]);
			DWORD ret = wait_for_multiple_objects_enhanced(i + 1, hObjects, TRUE, 10000, FALSE);
			ASSERT_INT_EQ(ret, WAIT_OBJECT_0_ENHANCED);
		}
	...
	{
		TEST_START("Signal: Wait-all with latent object in select positions (WAIT_TIMEOUT_ENHANCED)");
		TEST_RESOURCES(TRUE);
		...
		for (int i = 0; i < objects_size; i++) {
			...
			if (select == 0 || select == 1 || select == MAXIMUM_WAIT_OBJECTS - 1 || select == MAXIMUM_WAIT_OBJECTS - 2)
			{
				...
				DWORD ret = wait_for_multiple_objects_enhanced(objects_size, hObjects, TRUE, 250, FALSE);
	...
	{
		TEST_START("Signal: Wait-all with async threads in select object counts (WAIT_OBJECT_0_ENHANCED)");
		TEST_RESOURCES(TRUE);

		HANDLE threads[ARRAYSIZE(hObjects)];
		for (int i = 0; i < objects_size; i++) {
			DWORD select = i % MAXIMUM_WAIT_OBJECTS;
			if (select == 0 || select == 1 || select == MAXIMUM_WAIT_OBJECTS - 1 || select == MAXIMUM_WAIT_OBJECTS - 2) {
				for (int j = 0; j < i; j++) threads[j] = CreateThread(NULL, 0, signal_test_thread_random_sleep, hObjects[i], 0, NULL);
				DWORD ret = wait_for_multiple_objects_enhanced(objects_size, hObjects, TRUE, 10000, FALSE);
				ASSERT_INT_EQ(ret, WAIT_OBJECT_0_ENHANCED);
	...
	{
		TEST_START("Signal: Wait-any with async set in positions 1-300 (WAIT_OBJECT_0_ENHANCED offset)");
		TEST_RESOURCES(TRUE);

		for (int i = 0; i < objects_size; i++) ResetEvent(hObjects[i]);
		for (int i = 0; i < objects_size; i++) {
			CloseHandle(CreateThread(NULL, 0, signal_test_set_event, hObjects[i], 0, NULL));
			DWORD ret = wait_for_multiple_objects_enhanced(objects_size, hObjects, FALSE, 10000, FALSE);
	...
	{
		TEST_START("Signal: Wait-any with abandoned set in positions 1-300 (WAIT_ABANDONED_0_ENHANCED offset)");
		TEST_RESOURCES(TRUE);
