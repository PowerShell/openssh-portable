...
<#
.SYNOPSIS
    Run Attack Surface Analyzer test locally using Docker to analyze OpenSSH MSI installation.

.DESCRIPTION
    This script runs Attack Surface Analyzer in a clean Windows container to analyze
    the attack surface changes when installing OpenSSH MSI. It takes a baseline
    snapshot, installs the MSI, takes a post-installation snapshot, and exports the
    comparison results.

.PARAMETER MsiPath
    Path to the official signed OpenSSH MSI file to test. This must be a released,
    signed MSI from the official OpenSSH releases.
...
.EXAMPLE
    .\Run-AttackSurfaceAnalyzer.ps1 -MsiPath "C:\path\to\OpenSSH-Win64-v10.0.0.0.msi"

.EXAMPLE
    .\Run-AttackSurfaceAnalyzer.ps1 -MsiPath ".\OpenSSH-Win64-v10.0.0.0.msi" -OutputPath "C:\asa-results"

.NOTES
    Requires Docker Desktop with Windows containers enabled.
    Requires an official signed OpenSSH MSI file from a released build.
    ...
    - The MSI must be from an official OpenSSH release
...
if (-not (Test-MsiSignature -MsiPath $MsiPath)) {
    Write-Log "MSI signature verification failed. Only official signed OpenSSH MSIs are supported." -Level ERROR
    Write-Log "Please download an official OpenSSH MSI from: https://github.com/PowerShell/Win32-OpenSSH/releases" -Level ERROR
    exit 1
}
